...
with open("client/src/components/TeachingMaterials.tsx", "r", encoding="utf-8") as f:
    content = f.read()
...
# Load
old_load = """        const loadedLinks: { [key: number]: string } = {};
        for (let w = 1; w <= 14; w++) {
...
new_load = """        const loadedLinks: { [key: number]: string[] } = {};
        for (let w = 1; w <= 14; w++) {
          const key = `teaching_material_week_${w}`;
...
# handleLinkChange
old_change = """  const handleLinkChange = (week: number, value: string) => {
    setLinks(prev => ({ ...prev, [week]: value }));
  };"""
new_change = """  const handleLinkChange = (week: number, index: number, value: string) => {
    setLinks(prev => {
...
# handleSave
old_save = """      // Update with new links
      for (let w = 1; w <= 14; w++) {
        const key = `teaching_material_week_${w}`;
...
new_save = """      // Update with new links
      for (let w = 1; w <= 14; w++) {
        const key = `teaching_material_week_${w}`;
...
# Render inputs
old_render = """            <div style={{ flex: 1, display: 'flex', gap: '8px' }}>
              <input
                className="no-print"
                type="url"
                placeholder="https://..."
...
new_render = """            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
...
with open("client/src/components/TeachingMaterials.tsx", "w", encoding="utf-8") as f:
    f.write(content)
print("done")
