...
# 1. PrintSeparator class rename
content = content.replace('const PrintSeparator = ({ number, title }: { number: number | string; title: string }) => (\n  <div className="print-all-section"', 'const PrintSeparator = ({ number, title }: { number: number | string; title: string }) => (\n  <div className="print-separator-section"')
content = content.replace('const PrintSeparator = ({ number, title }: { number: number | string; title: string }) => (\r\n  <div className="print-all-section"', 'const PrintSeparator = ({ number, title }: { number: number | string; title: string }) => (\r\n  <div className="print-separator-section"')

# 2. Add CSS
if '.print-separator-section {' not in content:
    content = content.replace('.print-all-section {', '.print-separator-section { page-break-after: always; break-after: page; background: white !important; color: black !important; }\n            .print-all-section {')

# 3. tabDefs
tab_defs_new = """const tabDefs = [
...
content = re.sub(r'const tabDefs = \[.*?\];\s*const courseCode', tab_defs_new, content, flags=re.DOTALL)

# 4. panels
panels_new = """// Panel 0: Cover Page
...
content = re.sub(r'// Panel 0: Cover Page.*?(?=const htmlContent = `)', panels_new, content, flags=re.DOTALL)
