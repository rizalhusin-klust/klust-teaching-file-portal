...
// 4. Fix App.tsx handleUpdateAttendance
content = content.replace("status: 'Y' | 'N' | 'MC') => {", "status: 'Y' | 'N' | 'MC' | '') => {");

// 5. Fix CourseInfo type
content = content.replace("course_code: string;\n  course_name: string;", "course_code: string;\n  course_name: string;\n  program_name?: string;\n  program_abb?: string;");

fs.writeFileSync('client/src/App.tsx', content, 'utf8');
console.log('App.tsx final fix completed!');
